...
```{r}
env <- st_read("/Users/christinasun/Desktop/env/env.shp")
...
data(env)
coordinates(env) <- ~ x + y
proj4string(env) <- CRS("+init=epsg:28992")
env$response <- log(env$mean_temp)
...
# check smoothing
ks <- kriging_smooth_spherical(log(mean_temp) ~ 1, env)
y <- env$response
op <- par(mfrow = c(1, 2))
