...
temp <- ggplot() + geom_sf(color = 'grey', data = t) + geom_sf(aes(fill = temp_refit), data=na.omit(t)) + 
  geom_sf() +
  labs(title="temp") +
...
pm25 <- ggplot() + geom_sf(color = 'grey', data = t) + geom_sf(aes(fill = pm25_refit), data=na.omit(t)) + 
  geom_sf() +
  labs(title="PM2.5") +
  scale_fill_distiller(palette = "Greys",trans = "reverse") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black")) +theme_void()
