...
v <- variogram(mean_pm25 ~ 1+ Mdn_Inc_s  + Mdn_Inc_r +log(PpDnsty)  , jn)
...
# check smoothing:
ks <- kriging_smooth_spherical(mean_pm25 ~ 1+ Mdn_Inc_s  + Mdn_Inc_r +log(PpDnsty)  , jn)
y <- jn$mean_pm25
op <- par(mfrow = c(1, 2))
...
v <- variogram(mean_temp ~ 1 + Mdn_Inc_s  + Mdn_Inc_r +log(PpDnsty)  , jn)
v_fit <- fit.variogram(v, vgm("Sph")) #'vgm':generate a variogram model or add to an existing model.
v_f <- spherical_variogram(v_fit$psill[1], v_fit$psill[2], v_fit$range[2]) #Create spherical variograms
#Plot the variograms
op <- par(mfrow = c(1, 2))
...
# check smoothing
ks2 <- kriging_smooth_spherical(mean_temp ~ 1+ Mdn_Inc_s  + Mdn_Inc_r +log(PpDnsty)  , jn)
y2 <- jn$mean_temp
op <- par(mfrow = c(1, 2))
